...
def create_systemcarousel(elem: Element):
    qcontainer = QmlItem('Rectangle')
    qcontainer.props = get_defaults('system', elem.type, elem.name)
    ...
    qcarousel.props = get_defaults('system', elem.type + '__axis', elem.name + '__axis')
    ...
    def create_path(carousel_id: str, elem: Element) -> QmlItem:
        qpath = QmlItem('Path')
        ...
        if elem.params['type'].startswith('horizontal'):
            ...
            qcarousel.props['readonly property int itemMainLength'] = f"width / {elem.params['maxLogoCount']}"
        ...
        if elem.params['type'].startswith('vertical'):
            ...
            qcarousel.props['readonly property int itemMainLength'] = f"height / {elem.params['maxLogoCount']}"

        qpath.childs.append(qpathline)
        return qpath

    def create_delegate(carousel_id: str, elem: Element) -> QmlItem:
        qdelegate = QmlItem('Item')
        qdelegate.props = get_defaults('system', elem.type + '__axis_delegate', elem.name + '__axis_delegate')

        size = elem.params['logoSize']
        qdelegate.props.update({
            'width': f"{size.a} * root.width",
            'height': f"{size.b} * root.height",
            'scale': f"selected ? {elem.params['logoScale']} : 1.0",
        })
        qdelegate.extra_lines.append('Behavior on scale { NumberAnimation { duration: 200 } }')

        suffix = '__axis_delegate_image'
        qdelegate.childs.append(QmlItem('Image'))
        qdelegate.childs[-1].props = get_defaults('system', elem.type + suffix, elem.name + suffix)
        qdelegate.childs[-1].extra_lines = ['Behavior on opacity { NumberAnimation { duration: 120 } }']

        suffix = '__axis_delegate_text'
        qdelegate.childs.append(QmlItem('Text'))
        qdelegate.childs[-1].props = get_defaults('system', elem.type + suffix, elem.name + suffix)

        return qdelegate

    qcarousel.named_childs['path'] = create_path(qcarousel.props['id'], elem)
    qcarousel.named_childs['delegate'] = create_delegate(qcarousel.props['id'], elem)
